...
def on_focus_in(event):
    # Set the focus to another widget (e.g., a label) when Entry gets focus
    label.focus_set()

def on_escape_key(event):
    # Remove the focus from the Entry when Esc key is pressed
    root.focus()
...
label = tk.Label(root, text="Click here or press Esc")
label.pack()

entry.bind("<FocusIn>", on_focus_in)
entry.bind("<KeyPress-Escape>", on_escape_key)
