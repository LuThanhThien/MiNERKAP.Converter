...
class ImageIngestion:
    ...
    def initiate_image_obj(self, img:Image.Image, px_height:int=None, px_width:int=None,
                        xc:int=0, yc:int=0) -> None:
        try:
            self.img = img
    ...
    def crop_by_shape(self, shape:str) -> Image.Image:
        try:
            ...
            if self.px_height != None:
                self.px_width = SHAPE_RATIOS[shape] * self.px_height
            elif self.px_width != None:
                self.px_height = self.px_width / SHAPE_RATIOS[shape]
